...
fun real main() =
  let t_v1 = [2,3,4] in
  let t_v2 = rearrange((0),let a = rearrange((0),t_v1) in map(fn int (int x) => a[((size(0,rearrange((0),t_v1)) - x) - 1)],iota(size(0,rearrange((0),t_v1))))) in
  toFloat(let x = t_v2 in x[0])