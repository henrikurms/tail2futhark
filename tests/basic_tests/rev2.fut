...
fun real main() =
  let t_v1 = reshape((3,2),reshape1_int((3 * (2 * 1)),reshape(((size(0,map(fn int (int x) => (x + 1),iota(6))) * 1)),map(fn int (int x) => (x + 1),iota(6))))) in
  let t_v2 = rearrange((1,0),let a = rearrange((1,0),t_v1) in map(fn [int] (int x) => a[((size(0,rearrange((1,0),t_v1)) - x) - 1)],iota(size(0,rearrange((1,0),t_v1))))) in
  toFloat(let x = t_v2 in x[0,0])