...
fun real main() =
  ...
  let t_v2 = reshape((2,2,2),reshape1_int((2 * (2 * (2 * 1))),reshape(((size(0,[9,10,11,12,13,14,15,16]) * 1)),[9,10,11,12,13,14,15,16]))) in
  let t_v5 = map(fn [[int]] ([[int]] x,[[int]] y) => map(fn [int] ([int] x,[int] y) => map(+,zip(x,y)),zip(x,y)),zip(t_v1,t_v2)) in
  let t_v8 = map(fn [int] ([[int]] x) => map(fn int ([int] x) => reduce(+,0,x),x),t_v5) in
  let t_v11 = map(fn int ([int] x) => reduce(+,0,x),t_v8) in
  let t_v14 = reduce(+,0,t_v11) in
  toFloat(t_v14)