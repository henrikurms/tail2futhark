...
fun real main() =
  ...
  let t_v2 = reshape((2,2),reshape1_int((2 * (2 * 1)),reshape(((size(0,[5,6,7,8]) * 1)),[5,6,7,8]))) in
  let t_v5 = map(fn [int] ([int] x,[int] y) => map(+,zip(x,y)),zip(t_v1,t_v2)) in
  let t_v8 = map(fn int ([int] x) => reduce(+,0,x),t_v5) in
  let t_v11 = reduce(+,0,t_v8) in
  toFloat(t_v11)