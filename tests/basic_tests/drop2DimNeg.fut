...
fun real main() =
  let t_v0 = reshape((2,4),reshape1_int((2 * (4 * 1)),reshape(((size(0,map(fn int (int x) => (x + 1),iota(8))) * 1)),map(fn int (int x) => (x + 1),iota(8))))) in
  let t_v1 = reshape((if (0 <= (size(0,t_v0) - if (negi(1) <= 0) then -negi(1) else negi(1))) then (size(0,t_v0) - if (negi(1) <= 0) then -negi(1) else negi(1)) else 0,size(1,t_v0)),drop1_int((negi(1) * (size(1,t_v0) * 1)),reshape(((size(0,t_v0) * (size(1,t_v0) * 1))),t_v0))) in
  toFloat(let x = map(fn int ([int] x) => reduce(+,0,x),t_v1) in x[0])