...
fun real main() =
  let t_v0 = [1,2,3] in
  let t_v1 = reshape((3,2),reshape1_int((3 * (2 * 1)),reshape(((size(0,[9,8,7,6,5,4]) * 1)),[9,8,7,6,5,4]))) in
  let t_v2 = rearrange((1,0),concat([rearrange((0),t_v0)],rearrange((1,0),t_v1))) in
  let t_v3 = reshape((if (1 <= 0) then -1 else 1,size(1,t_v1)),take1_int((1 * (size(1,t_v1) * 1)),reshape(((size(0,t_v1) * (size(1,t_v1) * 1))),t_v1))) in
  toFloat(let x = map(fn int ([int] x) => reduce(+,0,x),t_v3) in x[0])