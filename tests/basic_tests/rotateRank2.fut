...
fun real main() =
  let t_v0 = reshape((2,3,4),reshape1_int((2 * (3 * (4 * 1))),reshape(((size(0,map(fn int (int x) => (x + 1),iota(8))) * 1)),map(fn int (int x) => (x + 1),iota(8))))) in
  let t_v1 = rearrange((2,1,0),let a = rearrange((2,1,0),t_v0) in map(fn [[int]] (int x) => a[((x + 2) % size(0,rearrange((2,1,0),t_v0)))],iota(size(0,rearrange((2,1,0),t_v0))))) in
  toFloat(let x = t_v1 in x[0,0,0])