...
fun real main() =
  let t_v1 = reshape((2,4),reshape1_int((2 * (4 * 1)),reshape(((size(0,map(fn int (int x) => (x + 1),iota(8))) * 1)),map(fn int (int x) => (x + 1),iota(8))))) in
  let t_v2 = reshape((if (0 <= (size(0,t_v1) - if (3 <= 0) then -3 else 3)) then (size(0,t_v1) - if (3 <= 0) then -3 else 3) else 0,size(1,t_v1)),drop1_int((3 * (size(1,t_v1) * 1)),reshape(((size(0,t_v1) * (size(1,t_v1) * 1))),t_v1))) in
  toFloat(reduce(+,0,map(fn int ([int] x) => reduce(+,0,x),t_v2)))