...
fun real main() =
  let t_v1 = let a = [boolToInt(True),2,3,4] in map(fn int (int x) => a[((x + 2) % size(0,[boolToInt(True),2,3,4]))],iota(size(0,[boolToInt(True),2,3,4]))) in
  toFloat(let x = t_v1 in x[0])