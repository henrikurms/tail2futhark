...
That is, $reshape([2,3],[1,2,3])$ evaluates to $[[1,2,3],[1,2,3]]$. Note that this is not how arrays are represented in TAIL. Instead of using nested brackets, to represent the dimensions, arrays in TAIL are represented with a shape as for instance in $[1,2,3,4,5,6]^{[2,3]}$). However, using this representation can make what happens less obvious, this we use the nested brackets representation insted. 
