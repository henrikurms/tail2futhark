...
There is however some specific implementation details that 

% friske navne - impl detalje - 
% indput fra fil / standard
% 

The compiler itself is implemented in the file {\tt src/Tail2Futhark/Compile.hs} and exports one function:
\begin{lstlisting}[language=haskell]
compile :: Options -> T.Program -> F.Program
\end{lstlisting}

The {\tt compile} function produces a Futhark program given options and a TAIL program. Since a TAIL proram is an expression it calls
a function called {\tt compileExpression} on the TAIL program. The resulting Futhark expression is then made the body of the main
function in the Futhark program.

The {\tt compileExpression} function pattern matches on the TAIL ast to compile the expression, with the difficult case bieng an
operator application, in that case the {\tt compileOperator} function is called which matches on the operator names.
