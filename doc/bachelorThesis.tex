...
Some of the functions are created from our templates. First the function body is created as a Futhark expression. Then we make a Futhark function that is parametrised over a Futhark type. We represent such a fundtion with the type {\tt F.Type -> FunDecl}. We can even parametrise the bodies of the functions by giving them the type {\tt F.Type -> F.Exp}, this type is simply passed from the parametrised function when it is instantiated. To produce all the functions we simply map the parametrised versions over the basic types. 
...
The test framework compiles each file with a {\tt tail} extension in the directory to a {\tt fut} file of the same name. Then the Futhark program
...
We feel these sequentially executed benchmarks are relevant because the are parallel in their structure and should therefore execute efficiently on parallel
 ...
 % dataparallelisme, hvile ting er forskellige er vigtige for dem, fx memory på GPU. Tilgang på 
 % større forståelse for hvad der inpacter/har betydning for parallelisme

During this project we have learned the value of using a symbolic notation to work from and to present and reason about in the form of our compilation scheme. 
...
% Svar på om vi har opnået det. 

In this paper we have desribed relevant part of the two languages TAIL and Futhark. 
We have also presented a compilation between the two languages shown in an implementation independent mathermatical notation as well as an implementation of this scheme in Haskell and test of this implementation. 
