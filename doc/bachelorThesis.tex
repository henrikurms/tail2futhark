...
%% inlined libery functions to deal with cases where no futhark function existed or polymorphism was required
%%% explain WHY we chose liberyfunctions and what the alternative was. 
...
The scheme is expressed in a type of notation that uses a lot of short hand in order to be able to express the compilation of the different TAIL expressions in a consise manner. This short hand notation should help the reader of the scheme to be able to easily get an overview of the compilation while still keeping a high level of detail in the compilation scheme. 
The scheme also present a way to present the idea behind the compilation that is implementation independent.\\

The notation is inspired by smiiar notation used in other projects \cite{TorbenMogensen}\cite{MartinElsmanNotation} to describe compilation schemes but is not build on a spefic standard. \\
...
\subsection{The compilation strategy}
The general stragegy for compiling TAIIL expressions was to use as many of the built-in functions in Futhark to express them as possible and then rely on Futharks havy optimization to remove %(et eller andet med det overhead eller sådan noget som vi potentielt kan have skabt kva at vi stort set ikke har tænkt på hvad der er optimalt.)

In the cases where it was imposible to use built-in Futhark 
...
% Methods used
% Libery functions
%% behrænsning kunne have været hvis vi havde valgt at implementere faste liberyfunctions en for hver rank og type. 
