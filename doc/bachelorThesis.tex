...
%i tail kan man bruge readintvec file
%det kan ikke lige oversætte til futhark
%futhark kan tage input ved at tage arg i man fkt men gør det ved at læse input fra standard in 
%ikke det sto mindre skulle vil kunne læse filer for at kunne implementere nogle benchmarks
%vi har derfor lavet et hack som siger hvis de første udtryk i programmet er indlæsnings fkt bliver de oversat til args i futhark. dvs. hvis tail programmet læser input gør futhark det også. Det eneste der supproter det er readIntVecFile i TAIL.
%Vi tager ikke hensyn til at syntaxen for det de læser ind er forskellig fra tail og futhark.
%programmøren er selv ansvarlig for at hvis tail filen læser ind fra en fil må man pipe indholdet (muligvis til et andet format) til standard in når man læser futhark programmer
...
%% vi er nødt til at læse matricerne ind for ellers optimere futhark det væk..

\subsection{Matric multiplication}
The matrix multiplication benchmark takes a matrix and multiplies it with itself transposed. It then reduces the resulting matrix twice, once by using times and once by using plus (times is used on the outer dimention).
...
We have been able to compile the sunset to Futhark code but since no parallel backend was finished for either TAIL or Futhark until the last week of this project we have not been able to test our compilation up against a parallel backend. Instead we have used the sequential backend that exixtsted for both languages. \\
% hvilken betydning har dette for at vi kan sige om vores mål er nået eller ej? 

Because Futhark does not support polymorphism in its type system compiling the polymorphic TAIL created some difficulties. We had to make create library functions for each basic type and create our compilation so that we only used library functions in the one-dimentional case. Otherwise we would have had to make a function for each version of basic type and array shape and considering array shapes are an infinitve set that would create a lot problems. Therefore having polymofphism in Futhark would be a very good idea.\\

The compilation scheme we did in our shorthand notation worked well. It enabled us to focus on the conceptual compilation and not get lost in the implementation details while doing the project. It also adds an overview of the entire compilation (everything can be written in a page or two) that would have been difficult to have without it. 
% Methods used
% Library functions
