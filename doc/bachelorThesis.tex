...
However as the functionality of these functions is not somewhat different the work lies in creating a mapping that retains the parallelism in the original code in the target language.
...
The C-code is then made into an executable (using -xmod) and the comandline tool {\tt time} is used to mesure the execution time. Each benchmark is run 10 times each and then the average is reported. The benchmarks and the results of the benchmarks are listed in Table 1.
...
% Doing the project have tought us a lot about parallelism in general and data-parallelism in particular. The possibility of harnesting the power of parallelism without as a programmer having to think about parallelism. 
% Also we have discovered what a huge effect the type system of different languages can have. In our concrete example we had an issue with polymorphism. This made the compilation a lot less simple since we had to make design desisions on how to handle this in the best way and what the best way was. 
% We learned a lot from creating the compilation scheme. The way of thinking in that level of abstraction about the compilation independent of a implementation have been informating. You get an easier overview and can se connections that can be difficult to spot otherwise. Also, it is a method that can be used later in other projects. 
