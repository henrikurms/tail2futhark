...
Given two arrays it reshapes the flattened representation of the second array $a_2$ to the dimentions given by the first array so 
if we have $reshape([2,3],[1,2,3,4,5,6])$ it evaluates to $[[1,2,3],[4,5,6]]$. If $a_2$ is to long the elements not needed are droped i.e. $reshape([2,3],[1,2,3,4,5,6,7,8])$ would evaluate to the same as the first example. If $a_2$ is sorter than needed the elements of $a_2$ are repeted, for instance $reshape([2,3],[1,2,3])$ evaluates to $[[1,2,3],[1,2,3]]$.\\

Other interesting operators are {\tt take($i$,$a$)} and {\tt drop($i$,$a$)}. They return the an array containing the 1st to $i$'th element of $a$, and the array containing the $i$'th to n'th element of $a$ respectivly. If the array is multi-dimentional the operator works on the outermost dimention of the array i.e. $take(2,[[1,2],[3,4][5,6]])$ evaluates to $[[1,2],[3,4]]$. If the array contains to few elements {\tt take} pads the array with zeros, whereas {\tt drop} returns the empty array in case one tries to drop more elements than there are in $a$. \\

The operator {\tt snoc(a,e)} takes two arrays $a$ and $e$ and return an array where the i'th element of $e$ is appended onto the end of the i'th row of $a$. If there is too few elements in $e$ you get an error, except if there is only one element in $e$ then the operater evaluates to an array where the one elemenent from $e$ is appended onto each row of $a$.\\

The operator {\tt cons(e,a)} has very similar semantics as the {\tt snoc} operator. The ownly difference is that it appends the contents of $e$ not on the end but at the beginning of each row.\\

The operator {\tt cat($a_1$,$a_2$)} takes two arrays that have to have the same outer dimention and returns an array where the i'th element (for instance row if the array is two-dimentional) of $a_2$ is appened onto the end of the i'th element of $a_1$.\\

The {\tt transp} operator takes an array and return the transposed array i.e $transp([[1,2,3],[4,5,6]])$ would return $[[1,4],[2,5],[3,6]]$. If the array is multi-dimentional for instance a three-dimentional array with the shape $2x3x4$ the function woul return an array with the shape $4x3x2$.\\
...
The {\tt replicate} function given an integer $n$ and an array $a$ returns an array consistung of $n$ copies of $a$.
...
The {\tt reshape} function takes a number of dimentions $(dim_1,..., dim_n)$ and an array $a$ and returns an array where the elements of $a$ is reshaped into the shape specified by the list of dimentions. The number of elements in $a$ must be equal to the product of the dimentions i.e. $elements$ $of$ $a$ $=$ $dim_1 * ... * dim_n$.\\

The function {\tt transpose} take an array $a$ and return the transposed $a$. If {\tt transpose} is used on an array of more than two dimentions it transposes the array a little different than one might expect. For instance the reult of transposing an three-dimentional array with dimentions $2 x 3 x 4$ is not as one might expect an array with dimentions $4 x 3x 2$ but instead an array with dimentions $3x4x2$.\\
...
The function {\tt concat} takes two arrays and concatinates them by concatinating the row/elements of one array with another. The shape of the two arrays have to be the same except in the first dimention. 

The undocumented {\tt reshape} function takes as arguments a comma seperated list of dimentions (surrounded in parentheses) and and array. It then rearanges the shape of the array to the by the list specified. \\
