...
We will not present an overview of APL but only reference to it. For a language reference pleace refer to \cite{APLbook} and (Dyalog REFERENCE!!).
...
\subsubsection{Xor} Likewise there is no logical xor operation so we included it in the library:
...
\subsubsection{All other library functions}
...
\subsubsection{The take operator} 
...
\subsubsection{The drop operator} 

\subsubsection{The snoc operator}
...
\subsubsection{The cons operator}
...
divd			& divd.tail			& (4.1 / 2.3) + 4				& FAIL (more digits in one then the other)\\
