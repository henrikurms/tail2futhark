...
On the left hand side of the scheme is the TAIL functions, the right hand side contains the Futhark expression they are translated to, except of the expression is wraped in double brackets. An expression wraped in double brackets e.g. $\evals{x}$ signifies the compilation of the TAIL expression $x$. \\
...
\paragraph{Var, let-expressions} almost are equivalent to Futhark let-expressions. The only difference is that the type information that exists in TAIL are ignored in Futhark. 
% var, literals/constants, let, array litterals, neg

The letters $x$, $i$, $d$, and $c$ dentoes variables, integers, doubles, and chars respectively. They are all translated to a Futhark equivalent. BOOLEANS MANGLER!!!!!!\\
...
\paragraph{The take/drop operator} 
The {\tt take} and {\tt drop} operators in TAIL 
