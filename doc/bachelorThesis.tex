...
The parser we used for this project is created in previous project that also worked with compiling TAIL to a parallel backend \cite{APLACC}. 
The latest version of the parser can be found in the github repository: https://github.com/mbudde/aplacc.
We did therefore not create the parser our selves. Instead we used the exsisting one because even though small changes had to be done to bring it up to date with the version of TAIL we work with, it saves time, thereby enbeling us to focus on essential work for our own project namely the compilation.
...
The compilation of the three parallel higher-order operators has the same recursive structure.
...
The parser used in this project was made previuosly in another project \cite{APLACC}. We did not create the parser and therefore the parser is not a main part of this project.
Because of the ungoing development of the TAIL language however, a small number of changes had to be done to update the parser to work on the latest version of the language. 
We have forked the original repository to work on updating the parser.
The original parser has since been adapted to work with the new version of TAIL. For the latest verion of the parser see the github repository: https://github.com/mbudde/aplacc. 
...
The tests can be found in the {\tt tests/basic\_tests/} directory.
...
Maybe we would implement fewer operators and instead focus on analyzing the code generated by the compilers and
from that argue about the soundness of our conversion rules.

During this project we have learned the value of having a good notation to work from and to present and reason about.
...
The notation is also a good tool for communicating during the development of the ideas for compilation.

Also we have learned that the type system means a lot when compiling between languages. 
...
Our main goal has been to compile a subset of TAIL to Futhark while still preserving the parallelism in the original code. 
...
We feel these sequentially executed benchmarks are relevant because the are parallel in their structure and should therefore execute efficiently on parallel hardware.
