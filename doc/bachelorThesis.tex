...
The take1, drop1 and reshape1 functions implement the TAIL operators take, drop and reshape in the 1-dimensional case.
We will see later that they can be used to implement the multi-dimensional cases,
this is advantageous as we would otherwise need a separate library function for each
...
We have defined four take functions (one of each base type) that are generated from a template.
...
We have defined four reshape functions (one for each base type) that are generated from a template.
...
The compiler is as mentioned written in Haskell and is placed in \verb|{tail2Futhark/src/|
\verb|Tail2Futhark/Compile.hs|. The implementation is very close to the conceptual compilation presented in Section \ref{sec:scheme}.
...
We assume that the compiler producing TAIL code from APL programs returns the correct result. We have therefore used this result to compare our own test results to.\\
