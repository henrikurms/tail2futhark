...
NÆVN AT ALLE TEST KØRTE MED OK! så man faktisk siger det lykkedes...
...
The matrix multiplication program takes a matrix and multiplies it with itself transposed. It then reduces the resulting matrix twice, once by using times and once by using plus (times is used on the outer dimention).
...
The code in TAIL can be seen below. 

\begin{lstlisting}[breaklines=true]
let v1:[int]1 = readIntVecFile(['m','a','t','r','i','x']) in
let v2:[int]1 = readIntVecFile(['s','i','z','e']) in
let v3:[int]0 = first{[int],[1]}(v2) in
let v5:[double]2 = reshape{[double],[1,2]}([v3,v3],each{[int,double],[1]}(fn v4:[int]0 => divd(1.0,i2d(v4)),v1)) in
let v6:[double]2 = transp{[double],[2]}(v5) in
let v13:[double]3 = transp2{[double],[3]}([2,1,3],reshape{[double],[2,3]}([v3,v3,v3],v5)) in
let v19:[double]3 = transp2{[double],[3]}([1,3,2],reshape{[double],[2,3]}([v3,v3,v3],v6)) in
let v24:[double]2 = reduce{[double],[2]}(addd,0.0,zipWith{[double,double,double],[3]}(muld,v13,v19)) in
reduce{[double],[0]}(muld,1.0,reduce{[double],[1]}(addd,0.0,v24))
\end{lstlisting}
