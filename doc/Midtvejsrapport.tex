...
$\evals{\text{vrotate}_{[t,r]}(i,a)}$ & $=$ & map $($ fn {\tt =>} $x + i \; mod \; (\text{size}(0,a))) \; \text{iota}(\text{size}(0,a)) $  \\         

$\evals{\text{vreverse}_{[t,r]}(a)}$ & $=$ & map $($ fn {\tt =>} $a[\text{size}(0,a)-x-1, \text{iota}(\text{size}(0,a))$ , a = fresh \\         

$\evals{\text{reshape}_{[t,r_1,r]}(a_1,a_2)}$ & $=$ & reshape$(\evals{a_1},(\text{reshape1}[\text{size}
(0,a_1)*...*\text{size}(r_1,a_1), \text{reshape}(\text{size}
(0,a_2)*...*\text{size}(r_2,a_2), \evals{a_2})])) $\\
...
$\evals{\text{take}(i,a)}$ & $=$ & $reshape([|i|, size(1,a),..,size(r,a)], \text{take1}_t[|i|*size(1,a)*..*size(r,a)], \text{reshape}(\text{size}
(0,a)*...*\text{size}(r,a),\evals{a}))$\\

$\evals{\text{takeV}_t(d,a)}$ & $=$ & $\text{take1}_{\evals{t}}(\evals{d},\evals{a})$\\

$\evals{\text{drop}(i,a)}$ & $=$ & $reshape(size(0,a), \text{drop1}_t[i*size(1,a)*..*size(r,a)], \text{reshape}(\text{size}
(0,a)*...*\text{size}(r,a),\evals{a}))$\\
