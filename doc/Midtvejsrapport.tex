...
TAIL stands for Typed Array Intermidiate Language and is an intermidiate language for the array programming language APL. APL is an older language created in the 1960's by Kenneth E. Iverson. The central datatype of the APL language is a multidimentional array. The language is dynamicly typed and has a functional core supporting fist and second order functions. A noteworthy thing about APL is its extencive use of special caracters wich is used to denote the build in functions and operators. 

APL is still extencively in some arreas such as the finacial world where large code bases are still operational and being actively developed. \cite{Elsman0} \\

TAIL is created as an intermidiate language to be ''a practical and well-defined intermidate formate for targeting parallel-architectures through a large number of exsisting tools and frameworks.'' \cite{Elsman0}. 

The main datatype in TAIL is also a multidimantional array. Array types are in TAIL annotated with their ranks explicetly. 


The type system in TAIL 

  called shapes,  where the number of dimentions are the rank of the shape. 
A couple of special cases for this datatype is the vector type


\section{Futhark}
The futhark language is designed to \dots

\section{The difference between TAIL and futhark}
...
\begin{thebibliography}{widest entry}
  ...
  \bibitem[Elsman]{Elsman0} \textbf{Compiling a subset of APL...} Martin Elsman and Martin Dybdal
