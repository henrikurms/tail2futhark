...
TAIL was designed with the purpose of targeting parallel architectures, though no parallel backend for TAIL yet exists. TAIL can also be compiled efficiently into a C-like language \cite{ElsmanDybdal:Array:2014}%\cite{Elsman0}. \\
...
The basic types in Futhark is {\tt int}, {\tt char}, {\tt bool}, {\tt real} and regular arrays. The type system of Futhark does not support polymorphism, however the build-in SOACs work on all regular arrays created of the basic types. 
...
The parser that we make use of was created in a precius project \cite{APLACC}. In the time between that project finished and ours commenced the TAIL language had been updated with new features and we therefore had to update the parser so that it 
...
\subsection{Creating a compilation scheme}
We will use the creation of a more formal compilation scheme using a form of mathematical nottation as a method to express the ideas behind the compilation we have done and also a method to remove the compilation strategy and scheme from implementation details making it independent of the concrete implementation of the compiler. 
...
The project is comming along as planed and we are keeping to our time schedule that can be seen in Figure \ref{fig:gantt}. It took shorter time than expected to adabt the parser to work with the latest version of TAIL and we therefore started on the implementation of the TAIL functions in the compiler a few days before sceduled.  We are done with implementing half of the functions in TAIL to Futhark source code.

\begin{figure}%[p]
    \centering
    \includegraphics[width=16cm]{midvejsgantt3.png}
    \caption{Gantt chart of the project activities. The remaining time of the project is highlighted by a red box.}
    \label{fig:gantt}
\end{figure}

The remainder of the time in the project we will focus on getting the last functions implemented and tested. We will also focus on creating a more formal translation/compilation scheme in order to learn how use a more mathematic semantic to describe the translation/compilation and in order to present the compilation in a way that is independent from the implementation. 
